...
The recipe that actually rotates it: **replace the `Secret` construct.** Rename its construct id (`OriginVerifySecret` → e.g. `OriginVerifySecretV2`) so CloudFormation mints a brand-new secret with a new ARN. Because both dynamic references are keyed on the ARN, both the frontend and backend stacks pick up a template diff on the next `make deploy` (`cdk deploy '**'`) — the WAF byte-match and the CloudFront origin header move together, atomically, in one deploy. Expect the same brief 403 window the original lockdown rollout had ((a) above): the backend stack (WAF rule) finishes before the frontend stack's CloudFront distribution has finished propagating the new header.

**Zero-downtime option.** Split the rotation across two deploys: first, widen the WAF rule's byte-match into an `or_statement` that accepts *both* the old and the new secret values, and deploy — every caller is now accepted regardless of which stack updates first. Once the new header is confirmed live everywhere, remove the old value from the `or_statement` in a second deploy.
