...
- `mypy-type-checker.importStrategy: "fromEnvironment"` (set in all four folder roots) makes the mypy extension run the venv's own mypy instead of its bundled copy. The default `useBundled` strategy injects the extension's `bundled/libs` onto the subprocess's import path, and the `typing_extensions.py` shipped there predates 4.14 — so the `pydantic.mypy` plugin fails to load with `cannot import name 'Sentinel' from 'typing_extensions'` (the stale bundled copy shadows the venv's current one; [microsoft/vscode-mypy#380](https://github.com/microsoft/vscode-mypy/issues/380)). `fromEnvironment` skips the bundled path entirely, which is also the right posture here on principle: mypy is version-pinned in `pyproject.toml`, and the editor should diagnose with exactly the pinned version that pre-commit and CI run, not whatever the extension bundles.
