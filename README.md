...
- **Coverage in the editor** comes from the Testing panel's **Run with Coverage** action (the run-with-coverage icon, or right-click a suite → *Run with Coverage*), which renders line/branch coverage in the editor gutters and the Test Coverage view. No configuration is needed: for coverage runs the Python extension injects its own `--cov=. --cov-branch` (it only defers to `pytestArgs` if you put `--cov` args there — don't, or coverage re-enters every discovery run), and the `addopts` strip above is what lets a coverage run of a *subset* report numbers instead of failing the 100% gate. **Which suite runs where follows the venv split** (see [Editor setup](#editor-setup-vs-code)): from the plain root folder the interpreter is `.venv`, so the CDK suites run and report `hello_world/` construct coverage (the one place that's measured — the CLI gate only ever measures `lambda/`), while the unit suite **skips cleanly** (every test that needs the Lambda runtime is guarded by `pytest.importorskip`, so you get `80 passed, 23 skipped`, not errors). To run *and* cover the unit suite, open the `tests/unit` root from `practice.code-workspace` — it uses `.venv-lambda` and shows `lambda/app.py` at 100%.
