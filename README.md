...
**(b) Manual secret rotation — no rotation Lambda, by design.** `OriginVerifySecret` carries no automatic rotation (`AwsSolutions-SMG4` and the matching NIST/HIPAA rotation rules are acknowledged with this rationale). Rotating it means updating two CFN-managed resources together — the CloudFront origin's custom header and the WAF rule's byte-match value — and a rotation Lambda would have to mutate both *outside* CloudFormation, which is drift CDK can't reconcile back. The supported recipe is simpler: put a new value into the secret (console, CLI, or let Secrets Manager generate one), then `make deploy` (or `cdk deploy '**'`) — both consumers resolve the secret via a CFN dynamic reference, so a normal deploy picks up the new value on both sides atomically, with no drift.
