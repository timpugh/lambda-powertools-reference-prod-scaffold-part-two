...
Code deploys get progressive delivery with automatic, alarm-driven rollback in `infrastructure/hello_world_app.py`, gated by `is_production_env` (canary in prod, fast in dev) — the machinery exists in both shapes, only the rollout speed differs.
