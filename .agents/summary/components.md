...
| Other workflows | `codeql.yml`, `scorecard.yml`, `dependency-audit.yml`, `dependabot-auto-merge.yml`, `pr-title.yml` (Conventional Commit gate on PR titles), `docs.yml` (Pages + coverage badge), `release.yml` (publishes GitHub Release from annotated tag) |
